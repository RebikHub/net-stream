...
export const Torrent = () => {
  ...
  const choseMovie = useCallback(
    (type: 'vlc' | 'html', nameMovie: string): void => {
      if (type === 'vlc') {
        ...
        getStreamLink(listMovies.hash, nameMovie)
          .then((res) => {
            setVideoSrc(res.url)
          })
          .catch(catchError)
      ...
      <div className={css.main}>
        <div className={css.controls}>
          ...
          <div className={css.buttons}>
            ...
            <button
              onClick={() => {
                if (listMovies.hash) stop(listMovies.hash)
              }}
            >
              Stop
            </button>
          ...
          {videoSrc ? (
            <div className={css.videoWrapper}>
              <div className={css.closeVideo} onClick={() => setVideoSrc(null)}>
                X
              </div>
              ...
              {listMovies.list.length === 1 ? (
                <div className={css.item}>
                  ...
                  <div className={css.itemBtn}>
                    <span
                      onClick={() => choseMovie('vlc', listMovies.list[0].name)}
                    >
                      смотреть через VLC
                    </span>
                    <span
                      onClick={() =>
                        choseMovie('html', listMovies.list[0].name)
                      }
                    >
                      смотреть через HTML
                    </span>
                ...
                <div className={css.list}>
                  {listMovies.list.map((item: any) => (
                    <div key={item.name} className={css.item}>
                      {item.name}
                      <div className={css.itemBtn}>
                        <span onClick={() => choseMovie('vlc', item.name)}>
                          смотреть через VLC
                        </span>
                        <span onClick={() => choseMovie('html', item.name)}>
                          смотреть через HTML
                        </span>
                      </div>
                    </div>
